...
Upgraded with 6D rotations, FBX/BVH handling, root trajectory tracking, and MDN architecture
"""
...
save_path = "results_Jason_Sherise_Take1_MDN/"
...
for i, (mocap_file_dancer1, mocap_file_dancer2) in enumerate(mocap_files):
    ...
    if not os.path.exists(d1_path):
        print(f"Warning: Mocking data because {d1_path} not found.")
        continue
...
# Stop if no data loaded (e.g. dummy environment)
if len(all_mocap_data_dancer1) == 0:
    print("Exiting cleanly for testing environment.")
    sys.exit()
...
class TransformerMDN(nn.Module):
    def __init__(self, mocap_dim, embed_dim, num_heads, num_encoder_layers, num_decoder_layers, dropout_p, pos_encoding_max_length, num_mixtures=5):
        super().__init__()
        ...
        self.mdn = MDNLayer(embed_dim, mocap_dim, num_mixtures)

    def forward(self, mocap_data_src, mocap_data_tgt, return_sequence=False):
...
transformer = TransformerMDN(mocap_dim=input_dim, embed_dim=transformer_embed_dim, num_heads=transformer_head_count, 
                          num_encoder_layers=transformer_layer_count, num_decoder_layers=transformer_layer_count, 
                          dropout_p=transformer_dropout, pos_encoding_max_length=pos_encoding_max_length, num_mixtures=mdn_num_mixtures).to(device)
...
def train(train_dataloader, test_dataloader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for train_batch in train_dataloader:
            ...
            _loss, _nll_loss, _pos_loss, _rot_loss = train_step(X_batch, y_batch, use_teacher_forcing)

            _train_loss_per_epoch.append(_loss.detach().cpu().numpy())
            _nll_loss_per_epoch.append(_nll_loss.detach().cpu().numpy())
        ...
        if epoch % model_save_interval == 0 and save_weights == True:
            torch.save(transformer.state_dict(), "{}/transformer_mdn_weights_epoch_{}".format(save_weights_path, epoch))
...
def create_pred_sequence(mocap_index, start_pose_index, pose_count, base_file_name, export_formats=["gif", "bvh", "fbx"]):
    ...
    for pI in range(0, pose_count - seq_length):
        ...
        with torch.no_grad():
            log_pi, mu, sigma = transformer(_in_d1, _in_d2, return_sequence=False)

        pred_pose = sample_mdn(log_pi, mu, sigma, pi_temperature=pi_temperature)[0, 0, :]
...
for i in range(4):
    pred_base = f"{save_anims_path}/pred_dancer2_mocap_{mocap_index}_start_{start_pose_index}_count_{pose_count}_epoch_{epochs}_test_run_{i}"
