"""
dance to dance transformer for duets
"""
...
seq_input_length = 64 # same for mocap and audio
...
seq_input_output_length = seq_input_length + seq_output_length
...
for i in range(len(all_mocap_data_dancer1)):
    ...
    mocap_rot_local_dancer1 = mocap_data_dancer1["motion"]["rot_local"]
    mocap_rot_local_dancer1 = np.reshape(mocap_rot_local_dancer1, (-1, pose_dim))
    
    mocap_rot_local_dancer2 = mocap_data_dancer2["motion"]["rot_local"]
    mocap_rot_local_dancer2 = np.reshape(mocap_rot_local_dancer2, (-1, pose_dim))

    print("shape ", mocap_rot_local_dancer1.shape)
    ...
    for valid_frame_range in valid_frame_ranges:
        ...
        for pI in np.arange(frame_range_start, frame_range_end - seq_input_output_length - 1):

            excerpt_dancer1 = mocap_rot_local_dancer1[pI:pI+seq_input_output_length]
            dancer1_data.append(excerpt_dancer1)
            
            excerpt_dancer2 = mocap_rot_local_dancer2[pI:pI+seq_input_output_length]
            dancer2_data.append(excerpt_dancer2)
...
class Transformer(nn.Module):
    ...
    def forward(self, mocap_data_src, mocap_data_tgt):
        ...
        #print("out 0 s ", out.shape)
        
        out = out[:, -1, :] # only last time step 
        
        #print("out 1 s ", out.shape)
...
yhat_batch = transformer(X_batch[:, :seq_input_length, :], y_batch[:, seq_input_length-1:seq_input_length, :])

print("X_batch s ", X_batch.shape)
print("y_batch s ", y_batch.shape)
print("yhat_batch s ", yhat_batch.shape)
...
def train_step(mocap_src, mocap_tgt, teacher_forcing):
    
    _input_dancer1 = mocap_src[:, 0:seq_input_length, :].detach().clone()
    _input_dancer2 = mocap_tgt[:, seq_input_length - 1, :].detach().clone()
    _target_dancer2 = mocap_tgt[:, seq_input_length, :].detach().clone()

    _loss_total = torch.zeros((1)).to(device)
    _norm_loss_total = torch.zeros((1)).to(device)
    _pos_loss_total = torch.zeros((1)).to(device)
    _rot_loss_total = torch.zeros((1)).to(device)

    for o_i in range(1, seq_output_length): 
        
        #print("o_i ", o_i, " out of ", seq_output_length)
        
        _input_dancer2 = _input_dancer2.reshape(-1, 1, pose_dim)
        
        #print("_input_dancer1 s ", _input_dancer1.shape)
        ...
        _pred_dancer2_norm = _pred_dancer2_norm.reshape(-1, 1, pose_dim)
        
        _pred_dancer2 = _pred_dancer2_norm * mocap_std_tensor + mocap_mean_tensor
        
        #print("_pred_dancer2 s ", _pred_dancer2.shape)
        
        _target_dancer2 = _target_dancer2.reshape(-1, 1, pose_dim)
        
        #print("_target_dancer2 s ", _target_dancer2.shape)

        _loss, _norm_loss, _pos_loss, _rot_loss = loss(_target_dancer2, _pred_dancer2) 
        ...
        _loss_total += _loss
        _norm_loss_total += _norm_loss
        _pos_loss_total += _pos_loss
        _rot_loss_total += _rot_loss
        ...
        _input_dancer1 = mocap_src[:, o_i:o_i + seq_input_length, :].detach().clone()
        _target_dancer2 = mocap_tgt[:,o_i + seq_input_length, :].detach().clone()
        
        if teacher_forcing == True:
            _input_dancer2 = mocap_tgt[:, o_i + seq_input_length, :].detach().clone()
        else:
            _input_dancer2 = _pred_dancer2.detach().clone()
    ...
    return _loss_total, _norm_loss_total, _pos_loss_total, _rot_loss_total

def test_step(mocap_src, mocap_tgt, teacher_forcing):
...
def train(train_dataloader, test_dataloader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for train_batch in train_dataloader:
            ...
            _loss, _norm_loss, _pos_loss, _rot_loss = train_step(X_batch, y_batch, use_teacher_forcing)
        ...
        for test_batch in test_dataloader:
            ...
            _loss, _, _, _ = test_step(batch_mocap, batch_audio, use_teacher_forcing)
...
def create_pred_sequence_anim(mocap_index, start_pose_index, pose_count, base_pose, file_name):
    ...
    start_seq_dancer1 = sequence_excerpt_dancer1[:seq_input_length, :]
    ...
    start_seq_dancer2 = sequence_excerpt_dancer2[seq_input_length-1:seq_input_length, :]
    start_seq_dancer2 = torch.from_numpy(start_seq_dancer2).to(torch.float32).to(device)
    start_seq_dancer2 = torch.reshape(start_seq_dancer2, (1, pose_dim))
    ...
    for pI in range(0, pose_count - seq_input_length):
        
        print("pI ", pI, " out of ", (pose_count - seq_input_length))
        ...
        _input_dancer1 = sequence_excerpt_dancer1[pI:pI + seq_input_length, :]
        _input_dancer1 = torch.from_numpy(_input_dancer1).to(torch.float32).to(device)
        _input_dancer1 = torch.reshape(_input_dancer1, (seq_input_length, pose_dim))
        
        _input_dancer2 = _pred_dancer2.detach().clone()
        
        _input_dancer2_np = _input_dancer2.cpu().numpy()
        _input_dancer2_np = np.reshape(_input_dancer2_np, (joint_count, joint_dim))
        gen_sequence[pI] = _input_dancer2_np
...
create_ref_sequence_anim(mocap_index, start_pose_index, pose_count, "results/anims/ref_dancer1_mocap{}_start{}_count{}.gif".format(mocap_index, start_pose_index, pose_count), "results/anims/ref_dancer2_mocap{}_start{}_count{}.gif".format(mocap_index, start_pose_index, pose_count))
create_pred_sequence_anim(mocap_index, start_pose_index, pose_count, base_pose, "results/anims/pred_dancer2_mocap{}_start{}_count{}_epoch{}.gif".format(mocap_index, start_pose_index, pose_count, epochs))
...
def create_new_pred_sequence_anim(mocap_index, start_pose_index, pose_count, base_pose, file_name):
...
create_new_pred_sequence_anim(new_mocap_index, new_start_pose_index, new_pose_count, base_pose, "results/anims/pred_dancer2_new_mocap{}_start{}_count{}_epoch{}.gif".format(new_mocap_index, new_start_pose_index, new_pose_count, epochs))
